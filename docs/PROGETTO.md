...
- **Stack scelto: Tauri 2 + Svelte 5 + TypeScript + Tailwind v4 + Lucide (UI) + backend Rust.**
  - UI overhaul 2026-06-22: struttura ad app (barra laterale + viste Cards/Decks/Matches/Collection/Settings), tema scuro Tailwind, galleria mazzi con artwork + filtri (nome/formato/colori) + ordinamento, editor in vista separata, anteprima carta che segue il cursore. (Anticipa parte della Fase 9 — Rifinitura.)
  - Ricerca avanzata carte (vista Cards, 2026-06-22): filtri per colore (identità, sottoinsieme), tipo, rarità, legalità formato, intervallo costo di mana (comando `search_cards_advanced`). (Framework UI cambiato da React a Svelte 5 il 2026-06-21 su scelta dell'utente: più moderno/leggero, accettando ~+5-10% effort e qualche rischio in più. TypeScript mantenuto.)
...
4. **Analisi mazzo**: tutti i grafici e la validazione legalità. ✅ COMPLETATA (2026-06-21) — comando `analyze_deck` (curva di mana, pip colori, tipi, rarità, legalità) + UI con **Chart.js** (barre + ciambelle a tema, statistiche, legalità con menù formati). Legalità filtrata ai 7 formati Arena (standard/alchemy/pioneer/historic/timeless/brawl/standardbrawl). Fix import: carte ribilanciate Alchemy (prefisso "A-") abbinate per nome (set+numero punterebbe alla versione sbagliata). 5 test backend OK + test manuale utente OK (Brawl, Historic).
